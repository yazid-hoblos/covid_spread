#!/usr/bin/env bash
# Hadoop script for Workload B (case fatality ratio per country)
DATASET_PATH="$1"
...
if [ -n "$STREAMING_JAR" ]; then
  ...
  if [ -x "$HADOOP_CMD" ]; then
    "$HADOOP_CMD" jar "$STREAMING_JAR" \
      -files ${HS_DIR}/mapper_B.py,${HS_DIR}/reducer_B.py \
      -input "$HDFS_DIR/covid_data.csv" \
      -output "$HDFS_DIR/output" \
      -mapper "python3 mapper_B.py" \
      -reducer "python3 reducer_B.py"
  else
    ...
    java -jar "$STREAMING_JAR" \
      ...
      -input "$HDFS_DIR/covid_data.csv" \
      -output "$HDFS_DIR/output" \
      -mapper "python3 ${HS_DIR}/mapper_B.py" \
      -reducer "python3 ${HS_DIR}/reducer_B.py"
  fi

  if [ "${JOB_RC:-0}" -ne 0 ]; then
    echo "Streaming job failed with exit code ${JOB_RC}; falling back to local Python processor"
    if [ -f "${HS_DIR}/process_B.py" ]; then
      python3 "${HS_DIR}/process_B.py" "$DATASET_PATH"
      exit $?
    else
      echo "Fallback processor ${HS_DIR}/process_B.py not found; exiting with original code ${JOB_RC}" >&2
      exit $JOB_RC
    fi
  fi
else
  ...
  if [ -f "${HS_DIR}/process_B.py" ]; then
    ...
    echo "Error: fallback processor ${HS_DIR}/process_B.py not found" >&2
    exit 2
  fi
fi
